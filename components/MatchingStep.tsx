...
import type { Match, UploadedFile, ColumnInMatch, ChatMessage, AIReview, ColumnData } from '../types';
import { MatchStatus, MatchSource, AIRecommendedAction } from '../types';
...
import { suggestMatchesForUnmatchedColumns, reviewAllPendingMatches, getCompletenessScore, generateColumnProfile } from '../services/geminiService';
...
export const MatchingStep: React.FC<MatchingStepProps> = ({ initialMatches, unmatchedColumns: initialUnmatched, files: initialFiles, onProceed }) => {
  ...
  // AI Settings
  const [apiCallDelay, setApiCallDelay] = useState(2); // seconds
  const [maxRetries, setMaxRetries] = useState(0); // 0 for unlimited
...
Your only job is to translate user requests into tool calls. Be direct and efficient.`;
  ...
  useEffect(() => {
    if (chat) {
        chat.config.systemInstruction = getSystemInstruction();
    }
  }, [getSystemInstruction, chat]);
...
// FIX: Add Array.isArray guard before spreading `columns` to prevent runtime error.
      ...
      if (releasedColumns.length > 0) {
        ...
        setUnmatchedColumns(prev => {
            ...
            allReleasedColsMap.forEach((cols, file) => {
                ...
                newUnmatched.set(file, [...new Set([...existing, ...cols])]);
            });
            return newUnmatched;
        });
      }
...
// FIX: Add Array.isArray guard before spreading `columns` to prevent runtime error.
      ...
      const response = await (async () => {
        switch(toolName) {
            case 'review_all_matches': {
                ...
                const reviews = await reviewAllPendingMatches(pending, files);
            ...
            case 'find_new_matches': {
                const newSuggestions = await suggestMatchesForUnmatchedColumns(unmatchedColumns, files, matches);
            ...
            while (true) {
                try {
                    ...
                    if (consecutiveFailures > 0) {
                        addAssistantMessage(`API call successful. Resetting delay to ${apiCallDelay} seconds.`);
                    }
                    ...
                    if (maxRetries > 0 && attempts >= maxRetries) {
                        addAssistantMessage(`Maximum number of retries (${maxRetries}) reached. Pausing autonomous mode.`);
                        throw new Error("Max retries reached");
                    }
                    
                    let currentDelaySeconds = apiCallDelay;
            ...
            try {
                ...
                while (true) {
                    ...
                    for (let i = 0; i < pendingForReview.length; i += REVIEW_BATCH_SIZE) {
                        ...
                        const batchReviews = await makeApiCallWithRetries(reviewAllPendingMatches, [batch, latestStateRef.current.files], `Review Batch ${i / REVIEW_BATCH_SIZE + 1}`);
                        
                        if (Array.isArray(batchReviews) && batchReviews.length > 0) {
                            reviewsThisPass.push(...batchReviews);
                        }
                        await delay(apiCallDelay * 1000);
                    }
                ...
                if (latestStateRef.current.unmatchedColumns.size > 0) {
                    const unmatchedCount = Array.from(latestStateRef.current.unmatchedColumns.values()).reduce((sum, cols) => sum + cols.length, 0);
                    ...
                    const newSuggestions = await makeApiCallWithRetries(suggestMatchesForUnmatchedColumns, [latestStateRef.current.unmatchedColumns, latestStateRef.current.files, latestStateRef.current.matches], "Matching");
  ...
  }, [isAutonomousMode, files, isFullyAutonomous, apiCallDelay, maxRetries]);
...
// FIX: Property 'length' does not exist on type 'unknown'. Add Array.isArray guard.
      if (!Array.isArray(matchToUpdate.columns) || matchToUpdate.columns.length <= 2) {
          ...
          setMatches(prev => prev.map(m => {
              if (m.id === matchId) {
                  return {
                      ...m,
// FIX: Type 'unknown' must have a '[Symbol.iterator]()' method that returns an iterator. Add Array.isArray guard.
          ...
          setUnmatchedColumns(prev => {
              ...
              newUnmatched.set(columnToDrop.fileName, [...fileCols, columnToDrop.columnName]);
              return newUnmatched;
          });
      }
  ...
  const allUnmatchedCount = useMemo(() => {
    return Array.from(unmatchedColumns.values()).reduce((acc, cols) => acc + cols.length, 0);
  }, [unmatchedColumns]);
  ...
  return (
    ...
    <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
      ...
      <aside className="space-y-6">
        ...
        />
        <AISettingsControl
            apiCallDelay={apiCallDelay}
            setApiCallDelay={setApiCallDelay}
